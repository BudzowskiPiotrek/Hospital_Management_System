...
public class PanelGestionEmpleados extends JPanel {
	...
	public PanelGestionEmpleados() {
		...
		btnGuardarTurno.addActionListener(e -> {
			try {
				//Se ha cambiado los parametros de Date a LocalDate y de Time a LocalTime
				...
				if (dniEmpleado.isEmpty() || dniPaciente.isEmpty() || diaStr.isEmpty() || horaComienzoStr.isEmpty()
					...
					LocalDate diaTransformado = transformarFecha(diaStr);
					LocalTime horaIncioTransfomado = transformarHora(horaComienzoStr);
					LocalTime horaFinTransformado = transformarHora(horaFinalStr);

					Turno turno = new Turno(dniEmpleado, dniPaciente, diaTransformado, horaIncioTransfomado,
							horaFinTransformado);
					if (dbConnection.asignarTurno(turno)) {
						JOptionPane.showMessageDialog(this, "Turno asignado exitosamente.", "Éxito",
								JOptionPane.INFORMATION_MESSAGE);
						((CardLayout) cardsPanel.getLayout()).show(cardsPanel, "Tabla");
					} else {
						JOptionPane.showMessageDialog(this, "Error al asignar el turno.", "Error",
								JOptionPane.ERROR_MESSAGE);
					}
				}
			} catch (DateTimeException r) {
				JOptionPane.showMessageDialog(this, "Por favor, ingrese la fecha o la hora con el formato correcto.",
						"Error de Validación", JOptionPane.WARNING_MESSAGE);
			}
		});
